            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    return '<a class="cut"> <button class="btn btn-primary btn-sm"  onclick="del(\''+row.id+'#'+row.stuId+'\')">删除</button> </a>'+
                           '<a class="show"> <button class="btn btn-primary btn-sm"  onclick="look(\''+row.id+'#'+row.stuId+'\')">查看</button> </a>';
                }
            }]
        });
...
//点击新增
function goUpload(){
    window.location.href="knowledgeUpload.html"
}
...
function see(result) {
    ...
    $("#modal_text").html(rows.content);


    $("#modal_title").html(rows.title);
    $('#myModal').modal('show')
